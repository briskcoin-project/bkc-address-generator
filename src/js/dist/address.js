...
function INIT_AddressUtil() {
    ...
    function MakePrivateKey(bigint) {
        ...
        privateKey.push(WorkerUtils.IsTestnet() ? 0xEF : 0x99); // updated for bkc
        privateKey.reverse();
    ...
    function MakeLegacyAddress(keypair) {
        ...
        var ripemd_extended = [WorkerUtils.IsTestnet() ? 0x6F : 0x19]; // updated for bkc
    ...
    function MakeSegwitAddress(keypair) {
        ...
        var redeemscripthash = [WorkerUtils.IsTestnet() ? 0xC4 : 0x21]; // updated for bkc
    ...
    function MakeBech32Address(keypair) {
        ...
        var address = WorkerUtils.IsTestnet() ? "tb1" : "bc1"; // updated for bkc
        for (var i = 0; i < result.length; ++i)
            address += bech32Chars[result[i]];
        var checksum = Bech32CreateChecksum(WorkerUtils.IsTestnet() ? "tb" : "bc", result); // updated for bkc
